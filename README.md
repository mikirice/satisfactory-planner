...
- **細かい条件**: 代替レシピの取捨、原料上限（マップのノード数由来）、製造クロック上限、
  採掘クロック、Somersloop、既に手元にあるアイテムの持ち込み、産出の最大化
...
**Phase 6（保存・共有）＋クロック / Somersloop / 床面積の対応まで完了。**
ブラウザで目標を入力すると結果が読め、6シートの .xlsx が落とせて、
原料→最終製品の流れをフローチャートで追える。
製造・採掘のクロック、Somersloop、概算床面積も画面と Excel の両方に出る。
...
- `clockedPowerMW` / `totalClockedPowerMW` … クロックと Somersloop を適用した実値。**画面と Excel の主表示**

### Somersloop の扱い（`SolveInput.somersloops`）
...
- 通常変数と併存するので、Somersloop が足りない分は同じレシピの通常ステップが埋める
- **部分装着（フル未満）はスコープ外**。装着数を連続変数にすると産出倍率が変数になって
  ...
  「Somersloop を使う」という意思決定に対しては十分な近似になる
- 制約はクロック100%換算の稼働台数に張る。一方、使用数の表示は「**建てる台数** × スロット数」
  ...
  - 逆にクロック上限を上げると台数が減るので、実際に挿す個数は上限より少なくなる
    ...
    「余った Somersloop を活かしきれない」ことがある。**まずクロック上限を決めてから
    Somersloop 数を調整する**のが実用上の使い方
...
- 入力: 目標産出（日本語名のインクリメンタル検索）/ 既にあるアイテム（手持ち・別工場からの供給。
  ...
  建物最小）/ **製造クロックの上限（10〜250%）と使える Somersloop の数** /
...
- 結果: サマリー（総電力の幅表示＋製造・採掘・パワーシャード、Somersloop の使用数と上限、
  **概算床面積とファウンデーション枚数**、建物数、建設コスト、シンクポイント、副産物）/
  生産ステップ（機械種別グルーピング・「N台 @ X%」・シャード・Somersloop）/
  原料（上限比率・採掘機台数・純度別ノード・シャード）/
  アイテム収支（余剰・不足を色とラベルの両方で表示）
- シャード列・Somersloop 列は実際に使うときだけ出す（既定の設定では表を細く保つ）
...
| シート | 内容 |
| --- | --- |
| サマリー | プラン名・目標産出（最大化した行は要求欄が「最大化」＋最大産出レート）・電力（製造/採掘/合計）・クロックと Somersloop（上限・クロック適用後の電力・シャード・使用数）・建物数・床面積の概算・必要原料・既保有アイテムの投入（投入量と使用量）・シンクポイント・副産物・有効な代替レシピ |
| 建物リスト | 機械種別ごとのレシピ・稼働台数・建てる台数・クロック・パワーシャード・Somersloop・電力・寸法（幅/奥行/高さ/設置面積）・投入/産出 |
...
- ノード: 原料供給（採掘）/ 既保有アイテムの持ち込み（紫の破線枠＋「既保有」ラベルで採掘と区別）/
  生産ステップ（レシピ名・機械種別・建てる台数とクロック・電力・Somersloop 使用数・投入/産出）/
  出力（目標産出は橙、副産物は無彩色）
